# Makefile —— FluxLinux build orchestrator
...
bootstrap:
	...
	@if ! lsb_release -i | grep -qE "(Ubuntu|Debian|Tuxedo|FluxLinux)"; then \
	  echo "Error: Unsupported OS — only Ubuntu, Debian, Tuxedo or FluxLinux allowed"; \
	  exit 1; \
	fi
