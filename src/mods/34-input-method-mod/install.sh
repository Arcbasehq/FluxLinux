...
if [ "$CONFIG_IBUS_RIME" == "true" ]; then
    ...
    zip=https://gitlab.aiursoft.com/FluxLinux/FluxLinux-rime/-/archive/master/FluxLinux-rime-master.zip
    wget $zip -O FluxLinux-rime.zip && unzip -q -O UTF-8 FluxLinux-rime.zip && rm FluxLinux-rime.zip
    mkdir -p /etc/skel/.config/ibus/rime
    rsync -Aavx --update --delete ./FluxLinux-rime-master/assets/ /etc/skel/.config/ibus/rime/
    rm -rf ./FluxLinux-rime-master/
