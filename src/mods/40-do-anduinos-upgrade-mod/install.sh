...
print_ok "Adding new command to this OS: do_FluxLinux_upgrade..."
cat <<"EOF" > /usr/local/bin/do_FluxLinux_upgrade
#!/bin/bash
set -o pipefail

echo "Upgrading FluxLinux..."

VERSION=$(grep -oP "VERSION_ID=\"\K\d+\.\d+" /etc/os-release)
URL="https://www.FluxLinux.com/upgrade/$VERSION"
...
EOF
chmod +x /usr/local/bin/do_FluxLinux_upgrade
judge "Add new command do_FluxLinux_upgrade"
