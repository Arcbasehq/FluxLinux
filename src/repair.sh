...
#=================================================
# This file is used to repair FluxLinux by mounting
# the ISO and replacing system files. It is intended
# for use when the system is broken or corrupted.
#
# This file is ONLY compatible with FluxLinux installed
...
function clean_up() {
  ...
  sudo umount /mnt/FluxLinux_squashfs >/dev/null 2>&1 || true
  sudo rm -rf /mnt/FluxLinux_squashfs >/dev/null 2>&1 || true
  #sudo umount /mnt/FluxLinux_iso >/dev/null 2>&1 || true
  #sudo rm -rf /mnt/FluxLinux_iso >/dev/null 2>&1 || true
...
# Parse "FluxLinux 1.4.1 questing - Release amd64 (20251111)"
...
if [ ! -f "/etc/lsb-release" ]; then
    print_error "System /etc/lsb-release file not found. Is this an installed FluxLinux?"
    exit 1
fi
...
if command -v dracut &> /dev/null || dpkg -l dracut 2>/dev/null | grep -q "^ii"; then
    ...
    print_error "FluxLinux cannot repair systems using dracut because:"
    ...
    print_error ""
    print_error "Please use a clean FluxLinux installation or manually resolve the conflict."
    exit 1
fi
...
sudo mkdir -p /mnt/FluxLinux_squashfs
sudo mount -o loop,ro "$SQUASH_FILE" /mnt/FluxLinux_squashfs
...
curl -s https://gitlab.aiursoft.com/FluxLinux/init-server/-/raw/master/mirror.sh?ref_type=heads | bash
sudo apt update
judge "Update package mirrors"

print_ok "Updating Mozilla Team PPA..."
sudo rm -f /etc/apt/sources.list.d/mozillateam*
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/apt/sources.list.d/mozillateam* /etc/apt/sources.list.d/
sudo apt update
...
if [ ! -s "$PKG_TEMP_FILE" ]; then
    ...
    if xargs sudo apt install --no-install-recommends --allow-change-held-packages -y < "$PKG_TEMP_FILE" > /tmp/FluxLinux-fast-install.log 2>&1; then
        print_ok "Fast mode installation successful."
        rm -f /tmp/FluxLinux-fast-install.log
        ...
        PKG_INSTALL_LOG="/tmp/FluxLinux-pkg-install.log"
...
#=================================================
#            Part 6: Apply the modifications of FluxLinux
#=================================================
install_spg_clean() {
...
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/gnome-shell/extensions/ /usr/share/gnome-shell/extensions/
judge "Upgrade GNOME Shell extensions"

print_ok "Upgrading icon and theme files..."
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/icons/ /usr/share/icons/
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/themes/ /usr/share/themes/
judge "Upgrade icon and theme files"

print_ok "Installing Intel SOF Mod..."
sudo rsync -Aax --update /mnt/FluxLinux_squashfs/lib/firmware/intel/sof* /lib/firmware/intel/
sudo rsync -Aax --update /mnt/FluxLinux_squashfs/usr/local/bin/sof-* /usr/local/bin/
sudo rsync -Aax --update /mnt/FluxLinux_squashfs/usr/share/alsa/ucm2/ /usr/share/alsa/ucm2/
judge "Install Intel SOF Mod"

print_ok "Upgrading desktop backgrounds..."
sudo rsync -Aax --update /mnt/FluxLinux_squashfs/usr/share/backgrounds/ /usr/share/backgrounds/
sudo rsync -Aax --update /mnt/FluxLinux_squashfs/usr/share/gnome-background-properties/ /usr/share/gnome-background-properties/
judge "Upgrade desktop backgrounds"

print_ok "Upgrading APT configuration files..."
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/etc/apt/apt.conf.d/ /etc/apt/apt.conf.d/
judge "Upgrade APT configuration files"

print_ok "Upgrading APT preferences files..."
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/etc/apt/preferences.d/ /etc/apt/preferences.d/
...
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/gnome-session/sessions/ /usr/share/gnome-session/sessions/
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/wayland-sessions/ /usr/share/wayland-sessions/
judge "Upgrade session files"

print_ok "Upgrading pixmaps..."
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/pixmaps/ /usr/share/pixmaps/
judge "Upgrade pixmaps"

print_ok "Upgrading /etc/skel/ files..."
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/etc/skel/ /etc/skel/
judge "Upgrade /etc/skel/ files"

print_ok "Upgrading python-apt templates and distro info..."
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/python-apt/templates/ /usr/share/python-apt/templates/
sudo rsync -Aax --update --delete /mnt/FluxLinux_squashfs/usr/share/distro-info/ /usr/share/distro-info/
judge "Upgrade python-apt templates and distro info"

print_ok "Upgrading deskmon service..."
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/local/bin/deskmon /usr/local/bin/deskmon
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/systemd/user/deskmon.service /etc/systemd/user/deskmon.service
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/systemd/user/default.target.wants/deskmon.service /etc/systemd/user/default.target.wants/deskmon.service
...
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/local/bin/do_FluxLinux_upgrade /usr/local/bin/do_FluxLinux_upgrade
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/local/bin/do-FluxLinux-autorepair /usr/local/bin/do-FluxLinux-autorepair
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/local/bin/toggle_network_stats /usr/local/bin/toggle_network_stats
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/bin/add-apt-repository /usr/bin/add-apt-repository
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/lsb-release /etc/lsb-release
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/issue /etc/issue
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/issue.net /etc/issue.net
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/os-release /etc/os-release
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/lib/os-release /usr/lib/os-release
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/legal /etc/legal
sudo rsync -Aax /mnt/FluxLinux_squashfs/etc/sysctl.d/20-apparmor-donotrestrict.conf /etc/sysctl.d/20-apparmor-donotrestrict.conf
sudo rsync -Aax /mnt/FluxLinux_squashfs/var/lib/flatpak/repo/config /var/lib/flatpak/repo/config
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/share/plymouth/themes/spinner/bgrt-fallback.png /usr/share/plymouth/themes/spinner/bgrt-fallback.png
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/share/plymouth/themes/spinner/watermark.png /usr/share/plymouth/themes/spinner/watermark.png
sudo rsync -Aax /mnt/FluxLinux_squashfs/usr/share/plymouth/ubuntu-logo.png /usr/share/plymouth/ubuntu-logo.png
